...
TEST_CASE("Windows-only features")
{
	...
	_wremove(fn.data());
	REQUIRE_SYSTEM_ERROR(open_file(fn, "rt+"), ENOENT);
...
#else
	REQUIRE_SYSTEM_ERROR(open_file("fn", "rt"), EINVAL);
	REQUIRE_SYSTEM_ERROR(open_file("fn", "r, ccs=utf-8"), EINVAL);
#endif
}
